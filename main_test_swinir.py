...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        # save_img_lq = img_lq.data.squeeze().float().cpu().clamp_(0, 1).numpy()
        # if save_img_lq.ndim == 3:
        #     save_img_lq = np.transpose(save_img_lq[[2, 1, 0], :, :], (1, 2, 0))  # CHW-RGB to HCW-BGR
        # save_img_lq = (save_img_lq * 255.0).round().astype(np.uint8)  # float32 to uint8
        # cv2.imwrite(f'{save_dir}/{imgname}_SwinIR_lq.png', save_img_lq)
        # inference
        with torch.no_grad():
