...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        if self.input_resolution == x_size:#here
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        h, w = x.shape[-2:]
        return to_3d(self.body(to_2d(x)), h, w)
...
class GDFN(nn.Module):
    ...
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.project_in = nn.Conv2d(dim, self.md, kernel_size=1, bias=bias)
        self.dwconv = nn.Conv2d(self.md, self.md, kernel_size=3, stride=1, padding=1, groups=self.md, bias=bias)
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, bias):
        ...
        m = 3
        self.qkv = nn.Conv2d(dim, dim * m, kernel_size=1, bias=bias)
        self.qkv_dwconv = nn.Conv2d(dim * m, dim * m, kernel_size=3, stride=1, padding=1, groups=dim * m, bias=bias)
...
class DilatedFCN(nn.Module):
    def __init__(self, in_dim):
        ...
        self.fcn = nn.Sequential(
            nn.Conv2d(in_dim, 32, 3, dilation=1, padding=1),
            nn.ReLU(inplace=True),
            ...
            nn.Conv2d(32, 32, 3, dilation=2, padding=2),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 1, 3, dilation=3, padding=3),
            nn.ReLU(inplace=True)
        )
    def forward(self, x):
        return self.fcn(x)
...
class PAR(nn.Module):
    def  __init__(self, img_size=64, patch_size=1, in_chans=1,
                 embed_dim=30, depths=[6, 6, 6], num_heads=[6, 6, 6],
                 window_size=7, mlp_ratio=2, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='', resi_connection='1conv',
                 num_blocks=[1, 1, 1], heads=[2, 4, 8], ffn_expansion_factor=1,bias=False,
        ...
        self.est = DilatedFCN(in_dim=embed_dim)
        self.FeatureExtra = torch.nn.Sequential(
            ConvLayer1(1, 60, 3, stride=1),
            nn.ReLU(inplace=True),
            ConvLayer1(60, 30, 3, stride=1)
        )

        self.feature_fusion = nn.Sequential(
            ConvLayer1(60, 60, 3, stride=1),
            nn.ReLU(inplace=True),
            ConvLayer1(60, 30, 3, stride=1)
        )
        self.lines = nn.ModuleList([line() for _ in range(self.num_layers)])
        #####################################################################################################
        ################################### 1, shallow feature extraction ###################################
        self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        # self.conv_first = torch.nn.Sequential(
        #     ConvLayer1(num_in_ch, embed_dim, 3, stride=1),
        #     nn.ReLU(inplace=True),
        #     ConvLayer1(embed_dim, embed_dim, 3, stride=1)
        # )
        #####################################################################################################
        ...
        self.channelattlayer = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = nn.Sequential(
                *[TransformerBlock(dim=embed_dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                                   bias=bias, layer_norm_type=layer_norm_type
                                   ) for _ in range(num_blocks[0])]
                )
            self.channelattlayer.append(layer)
        self.reduce_chan = nn.Conv2d(embed_dim * 2, embed_dim, kernel_size=1, bias=bias)
        self.reduce_chan_1 = nn.Conv2d(embed_dim * 2, embed_dim, kernel_size=1, bias=bias)
    ...
    def forward_features_singlenoisemap(self, x, sigma):

        x_size = (x.shape[2], x.shape[3])
        mid = x
        for i, layer in enumerate(self.layers):
            x_ = x
            x = self.feature_fusion(torch.cat([x, sigma], dim=1))
        ...
        return x
    #every rstb calculate noisemap
    def forward_features_mulnoisemap(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            sigma = self.FeatureExtra(sigma_prediction)

            x = self.feature_fusion(torch.cat([x, sigma], dim=1))
            chanidx = int(i%2)
            if chanidx == 0:
                chanattlayer = self.channelattlayer[chanidx]
                x = self.reduce_chan(torch.cat([x, mid], dim=1))
                x_chan = chanattlayer(x)
            else:
                x = self.reduce_chan_1(torch.cat([x, x_chan], dim=1))
    ...
    def forward(self, x):
        ...
        res = self.conv_after_body(self.forward_features_mulnoisemap(x_first))  + x_first
...
if __name__ == '__main__':
    ...
    model = PAR(upscale=2, img_size=(height, width),
